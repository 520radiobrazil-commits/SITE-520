...
const Footer: React.FC<FooterProps> = ({ onShowAbout, onShowTerms, onShowPrivacy }) => {
  const homeUrl = 'https://radio520.com.br/';
  const urlWithUtm = `${homeUrl}?utm_source=radio520&utm_medium=site&utm_campaign=home`;
  const shareTitle = 'RADIO520.COM.BR - Esportes e Notícias';
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              <li><button onClick={onShowAbout} className="hover:text-white text-left">Sobre Nós</button></li>
              <li><a href="mailto:520radiobrazil@gmail.com" className="hover:text-white">Contato</a></li>
              <li><button onClick={onShowPrivacy} className="hover:text-white text-left">Política de Privacidade</button></li>
              <li><button onClick={onShowTerms} className="hover:text-white text-left">Termos de Serviço</button></li>
            </ul>
          </div>
          <div>
            <h3 className="text-white font-semibold mb-4">Siga-nos</h3>
            <div className="flex space-x-4 mb-6">
              <a href="https://x.com/radio_520" target="_blank" rel="noopener noreferrer" className="hover:text-white"><XIcon /></a>
              <a href="https://www.instagram.com/radio520oficial/" target="_blank" rel="noopener noreferrer" className="hover:text-white"><InstagramIcon /></a>
              <a href="https://www.facebook.com/share/1BS7uVusG5/?mibextid=wwXIfr" target="_blank" rel="noopener noreferrer" className="hover:text-white"><FacebookIcon /></a>
            </div>
            <h3 className="text-white font-semibold mb-4">Compartilhe o Site</h3>
            <div className="flex space-x-4">
               <a href={shareLinks.x} target="_blank" rel="noopener noreferrer" className="hover:text-white" aria-label="Compartilhar no X"><XIcon /></a>
               <a href={shareLinks.facebook} target="_blank" rel="noopener noreferrer" className="hover:text-white" aria-label="Compartilhar no Facebook"><FacebookIcon /></a>
               <a href={shareLinks.whatsapp} target="_blank" rel="noopener noreferrer" className="hover:text-white" aria-label="Compartilhar no WhatsApp"><WhatsAppIcon /></a>
               <a href={shareLinks.bluesky} target="_blank" rel="noopener noreferrer" className="hover:text-white" aria-label="Compartilhar no Bluesky"><BlueskyIcon /></a>
            </div>
          </div>
        </div>
